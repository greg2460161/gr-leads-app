...
const DIRECT_PRODUCTS = {
  bridging: { label: "Bridging Finance", color: "#c8952a", description: "Auction buyers, chain breaks, urgent property purchases", searchQuery: "property auction upcoming lots England Wales 2026 bridging finance needed" },
  ...
  mca: { label: "Merchant Cash Advance", color: "#ef4444", description: "Retail, hospitality and card-taking SMEs needing fast cash", searchQuery: "new restaurant retail business opening England Wales 2026" },
};
...
const CH_PROFILES = {
  ...
  all: { label: "All SMEs - Business Loans", sicCodes: [], description: "Any SME in England and Wales" },
};
...
function extractJSON(text) {
  ...
  const str = t.slice(start, end + 1);
  try { return JSON.parse(str); } catch (e) { throw new Error("JSON parse failed: " + e.message); }
}

async function getDirectLeads(apiKey, product) {
  const searchText = await callClaude(
    apiKey,
    "You are a research assistant. Search the web and return a plain text summary of what you find. Include company names, locations, and contact details where available.",
    `Search for: ${product.searchQuery}. Find real companies and individuals in England and Wales who likely need ${product.label}. Summarise what you find in plain text.`,
    true
  );

  const jsonText = await callClaude(
    apiKey,
    `You are a JSON generator. You output ONLY raw JSON with no markdown, no backticks, no explanation. Start your response with { and end with }.`,
    `Using this research, generate a JSON object for GR Commercial Finance lead tracking.

Research: ${searchText.slice(0, 800)}
  ...
  "leads": [
    {
      ...
      "signal": "Why they need ${product.label}",
      "estimatedDeal": "GBP 100,000",
      "urgency": "High",
      "qualificationScore": 80,
      "contactHint": "How to reach them",
      "emailSubject": "Subject line",
      "emailBodyCore": "2-3 sentences about their specific situation and how GR Commercial Finance can help with ${product.label}"
    }
  ],
  "searchSummary": "One sentence summary"
}

Rules: use only basic ASCII characters, no special quotes, no newlines inside string values, include 3 to 5 leads.`,
    false
  );
...
async function getReferralPartners(apiKey, partner) {
  const searchText = await callClaude(
    apiKey,
    "You are a research assistant. Search the web and return a plain text summary of what you find.",
    `Search for: ${partner.searchQuery}. Find real ${partner.label} firms in England and Wales that could become referral partners for a commercial finance broker. Summarise what you find in plain text.`,
    true
  );
  ...
  const jsonText = await callClaude(
    apiKey,
    `You are a JSON generator. You output ONLY raw JSON with no markdown, no backticks, no explanation. Start your response with { and end with }.`,
    `Using this research, generate a JSON object for GR Commercial Finance referral partner tracking.
  ...
  "partners": [
    {
      ...
      "referralOpportunity": "Why they would refer clients to GR Commercial Finance",
      "partnerScore": 75,
      "contactHint": "How to approach them",
      "emailSubject": "Subject line for partnership email",
      "emailBodyCore": "2-3 sentences introducing GR Commercial Finance as a referral partner, mentioning referral fees, and asking to connect"
    }
  ],
  "searchSummary": "One sentence summary"
}

Rules: use only basic ASCII characters, no special quotes, no newlines inside string values, include 3 to 5 partners.`,
    false
  );
...
async function enrichCompany(apiKey, company, profileLabel) {
  const jsonText = await callClaude(
    apiKey,
    `You are a JSON generator. You output ONLY raw JSON with no markdown, no backticks, no explanation. Start your response with { and end with }.`,
    `Generate outreach data for this UK company for GR Commercial Finance.
Company: ${company.companyName}
Location: ${company.location}
Incorporated: ${company.incorporatedOn}
Profile: ${profileLabel}
Companies House: company-information.service.gov.uk/company/${company.companyNumber}

Output this JSON exactly:
{
  "emailAddress": "info@companyname.co.uk",
  "emailConfidence": "Guessed",
  "signal": "One sentence why they need finance",
  ...
  "emailSubject": "Finance options for your business",
  "emailBodyCore": "2 sentences about how GR Commercial Finance can help this type of business with commercial finance"
}

Rules: use only basic ASCII characters, no special quotes, no newlines inside string values.`,
    false
  );
...
export default function App() {
  ...
  const handleSearch = async () => {
    ...
    try {
      if (mode === "direct") {
        ...
        const result = await getDirectLeads(apiKey, p);
        setLeads(result.leads || []);
        setSummary(result.searchSummary || "");
      } else {
        const p = REFERRAL_PARTNERS[activePartner];
        const result = await getReferralPartners(apiKey, p);
  ...
  const handleCHSearch = async () => {
    ...
    try {
      ...
      setChProgress(`Found ${companies.length} companies. Generating outreach emails (this takes a minute)...`);
      const toEnrich = companies.slice(0, 20);
      const enriched = [];
      for (let i = 0; i < toEnrich.length; i++) {
        setChProgress(`Writing email ${i + 1} of ${toEnrich.length}...`);
        const extra = await enrichCompany(apiKey, toEnrich[i], profile.label);
        enriched.push({ ...toEnrich[i], ...extra });
        if (i < toEnrich.length - 1) await new Promise(res => setTimeout(res, 300));
      }
      const remaining = companies.slice(20).map(c => ({
        ...c,
        emailAddress: "info@" + c.companyName.toLowerCase().replace(/[^a-z0-9]/g, "") + ".co.uk",
        emailConfidence: "Guessed",
        ...
        emailSubject: "Finance options for " + c.companyName,
        emailBodyCore: "We are GR Commercial Finance, specialist commercial finance brokers covering England and Wales. We help " + profile.description.toLowerCase() + " access competitive business finance. We would love to discuss how we can support your business.",
      }));
  ...
  const buildFullEmail = (lead) => {
    const body = lead.emailBodyCore || "";
    return body + "\n\n" + EMAIL_SIG;
  };

  const exportCSV = (leadsData, isReferral) => {
    if (!leadsData.length) return;
    const headers = isReferral
    ...
    const rows = leadsData.map(l => isReferral
      ? [l.companyName, l.contactName, l.location, l.emailAddress, l.emailConfidence, l.clientProfile, l.referralOpportunity, l.partnerScore, l.contactHint, l.emailSubject, buildFullEmail(l).replace(/\n/g, " ")]
      : [l.companyName, l.contactName, l.location, l.emailAddress, l.emailConfidence, l.signal, l.estimatedDeal, l.urgency, l.qualificationScore, l.contactHint, l.emailSubject, buildFullEmail(l).replace(/\n/g, " ")]
    );
  ...
  const copyEmailFull = (e, lead, i, setCopiedFn) => {
    e.stopPropagation();
  ...
  const LeadCard = ({ lead, i, expandedState, setExpandedState, copiedState, setCopiedState, isReferral }) => {
    ...
    const fullEmail = buildFullEmail(lead);
    return (
      <div onClick={() => setExpandedState(expandedState === i ? null : i)}
        style={{ background: "#0f0f16", border: "1px solid " + (expandedState === i ? "#c8952a44" : "#1c1c28"), borderRadius: 8, overflow: "hidden", cursor: "pointer", marginBottom: 8 }}>
          ...
          <div style={{ borderTop: "1px solid #1c1c28", padding: "13px 16px", background: "#09090d" }}>
            ...
            {lead.emailSubject && <div>
              ...
              <div style={{ background: "#0f0f16", border: "1px solid #1c1c28", borderRadius: 6, padding: "10px 13px", marginBottom: 8 }}>
                ...
                <div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.7, whiteSpace: "pre-wrap", borderTop: "1px solid #1c1c28", paddingTop: 8 }}>{fullEmail}</div>
              </div>
              <button onClick={e => copyEmailFull(e, lead, i, setCopiedState)}
                style={{ padding: "5px 14px", borderRadius: 4, border: "1px solid #1c1c28", background: copiedState === i ? "#14532d" : "transparent", color: copiedState === i ? "#86efac" : "#4b5563", cursor: "pointer", fontSize: 11 }}>
                {copiedState === i ? "Copied!" : "Copy full email"}
              </button>
            </div>}
          </div>
        )}
    ...
    <div style={{ fontFamily: "Arial,sans-serif", background: "#09090d", minHeight: "100vh", color: "#ddd8ce" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <input type="password" placeholder="sk-ant-api..." value={apiKey} onChange={e => setApiKey(e.target.value)}
              style={{ width: 150, background: "#09090d", border: "1px solid #1c1c28", borderRadius: 5, padding: "6px 10px", color: "#ddd8ce", fontSize: 12, outline: "none", fontFamily: "monospace" }} />
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: 10, color: "#374151", textTransform: "uppercase", letterSpacing: 1 }}>CH Key</span>
            <input type="password" placeholder="Companies House..." value={chApiKey} onChange={e => setChApiKey(e.target.value)}
              style={{ width: 150, background: "#09090d", border: "1px solid #1c1c28", borderRadius: 5, padding: "6px 10px", color: "#ddd8ce", fontSize: 12, outline: "none", fontFamily: "monospace" }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "#0f0f16", borderBottom: "1px solid #1c1c28", padding: "0 24px", display: "flex" }}>
        {[["direct", "Direct Leads", "#c8952a"], ["referral", "Referral Partners", "#2ac87a"], ["companies", "Companies House", "#f59e0b"]].map(([m, label, col]) => (
          <button key={m} onClick={() => { setMode(m); setLeads([]); setSummary(""); setError(""); }}
            style={{ padding: "12px 18px", border: "none", borderBottom: mode === m ? "2px solid " + col : "2px solid transparent", background: "transparent", color: mode === m ? col : "#4b5563", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
            {label}
          </button>
        ))}
      </div>
          ...
          <div style={{ background: "#0f0f16", border: "1px solid #1c1c28", borderRadius: 8, padding: "20px", marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              <div>
                ...
                <select value={chProfile} onChange={e => setChProfile(e.target.value)}
                  style={{ width: "100%", background: "#09090d", border: "1px solid #1c1c28", borderRadius: 5, padding: "8px 10px", color: "#ddd8ce", fontSize: 12, outline: "none" }}>
              ...
              <div>
                ...
                <input placeholder="e.g. Manchester, Bristol, Cardiff..." value={chLocation} onChange={e => setChLocation(e.target.value)}
                  style={{ width: "100%", background: "#09090d", border: "1px solid #1c1c28", borderRadius: 5, padding: "8px 10px", color: "#ddd8ce", fontSize: 12, outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ fontSize: 11, color: "#6b7280", display: "block", marginBottom: 4 }}>Keyword (optional)</label>
                <input placeholder="e.g. pizza restaurant, roofing contractor..." value={chQuery} onChange={e => setChQuery(e.target.value)}
                  style={{ width: "100%", background: "#09090d", border: "1px solid #1c1c28", borderRadius: 5, padding: "8px 10px", color: "#ddd8ce", fontSize: 12, outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ fontSize: 11, color: "#6b7280", display: "block", marginBottom: 4 }}>Max results</label>
                <select value={chResults} onChange={e => setChResults(Number(e.target.value))}
                  style={{ width: "100%", background: "#09090d", border: "1px solid #1c1c28", borderRadius: 5, padding: "8px 10px", color: "#ddd8ce", fontSize: 12, outline: "none" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <button onClick={handleCHSearch} disabled={chSearching}
                style={{ padding: "9px 24px", borderRadius: 5, border: "none", background: chSearching ? "#1c1c28" : "#f59e0b", color: chSearching ? "#4b5563" : "#000", cursor: chSearching ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 700 }}>
                {chSearching ? "Searching..." : "Search Companies House"}
              </button>
              {chLeads.length > 0 && <>
                <button onClick={() => setViewMode(v => v === "cards" ? "table" : "cards")}
                  style={{ padding: "8px 14px", borderRadius: 5, border: "1px solid #1c1c28", background: "transparent", color: "#4b5563", cursor: "pointer", fontSize: 11 }}>{viewMode === "cards" ? "Table" : "Cards"}</button>
                <button onClick={() => exportCSV(chLeads, false)}
                  style={{ padding: "8px 14px", borderRadius: 5, border: "1px solid #1c1c28", background: "transparent", color: "#4b5563", cursor: "pointer", fontSize: 12 }}>Export CSV</button>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                ...
                <tbody>{chLeads.map((lead, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #1c1c28", background: i % 2 === 0 ? "#09090d" : "#0c0c12" }}>
                    ...
                    <td style={{ padding: "8px 10px" }}><button onClick={e => copyEmailFull(e, lead, i, setChCopied)} style={{ padding: "3px 9px", borderRadius: 3, border: "1px solid #1c1c28", background: chCopied === i ? "#14532d" : "transparent", color: chCopied === i ? "#86efac" : "#4b5563", cursor: "pointer", fontSize: 10 }}>{chCopied === i ? "Copied" : "Copy"}</button></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
          )}
          {viewMode === "cards" && chLeads.map((lead, i) => (
          ...
          <div style={{ maxWidth: 1040, margin: "0 auto", padding: "20px 24px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, gap: 12, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {leads.length > 0 && <>
                  <button onClick={() => setViewMode(v => v === "cards" ? "table" : "cards")}
                    style={{ padding: "7px 12px", borderRadius: 5, border: "1px solid #1c1c28", background: "transparent", color: "#4b5563", cursor: "pointer", fontSize: 11 }}>{viewMode === "cards" ? "Table" : "Cards"}</button>
                  <button onClick={() => exportCSV(leads, mode === "referral")}
                    style={{ padding: "7px 14px", borderRadius: 5, border: "1px solid #1c1c28", background: "transparent", color: "#4b5563", cursor: "pointer", fontSize: 12 }}>Export CSV</button>
                </>}
                  ...
                  <tbody>{leads.map((lead, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: "1px solid #1c1c28", background: i % 2 === 0 ? "#09090d" : "#0c0c12" }}>
                        ...
                        <td style={{ padding: "8px 10px" }}><button onClick={e => copyEmailFull(e, lead, i, setCopied)} style={{ padding: "3px 9px", borderRadius: 3, border: "1px solid #1c1c28", background: copied === i ? "#14532d" : "transparent", color: copied === i ? "#86efac" : "#4b5563", cursor: "pointer", fontSize: 10 }}>{copied === i ? "Copied" : "Copy"}</button></td>
                      </tr>
                    );
                  })}</tbody>
                </table>
            ...
            {viewMode === "cards" && leads.map((lead, i) => (
